...
// Создает пулл потоков. Собирается список (tasks) задач по ячейкам. Симуляциия длится Settings.AGE_DURATION "дней".
// Каждый день запускается processDay() в котором в пул потоков передается список задач.
// Это действие повторяется ""количество ячеек" * Settings.DAILY_COUNT_ITERATIONS" количество раз.
...
// Потом запусает отрисовку острова и выводит статистику за день на экран.
